    ...
    <section ref={sectionRef} id='experience' className='py-24 px-4'>
      <div className='max-w-6xl mx-auto'>
        ...
        <div className='relative space-y-8'>
          <div className='hidden md:block absolute left-[15px] top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary via-accent to-transparent'></div>

          {experiences.map((exp, index) => (
            <div
              key={index}
              className={`relative bg-card p-6 md:p-8 md:ml-16 rounded-xl border border-border hover:border-primary/50 transition-all duration-300 hover:shadow-xl hover:shadow-primary/10 group opacity-0 ${
                isVisible ? `animate-fadeInUp stagger-${index + 1}` : ""
              }`}>
              <div className='hidden md:flex absolute -left-[49px] top-8 w-8 h-8 bg-gradient-to-br from-primary to-accent rounded-full items-center justify-center border-4 border-background shadow-lg'>
